...
class InventoryWizard(models.TransientModel):
    ...
    filter_by = fields.Selection([('by_product', 'Product'), ('by_category', 'Product Category')],
                                 string='Filter By')
    ...
    def get_products(self):
        ...
        if not self.filter_by:
            ...
            if self.filter_by == 'by_product':
                ...
                product_ids = product_pool.search([('categ_id', 'child_of', self.category_id.id),
                                                   ('type', '=', 'product')])
                if product_ids:
                    return product_ids
                else:
                    raise ValidationError(_("Product not found in selected category !!!"))
    ...
    def get_style(self):
        ...
        return [main_header_style, left_header_style, header_style, text_left, text_right,
                text_left_bold, text_right_bold, text_center]

    @api.multi
    def create_excel_header(self, worksheet, main_header_style,
                            left_header_style, text_left):
        ...
        worksheet.write_merge(row, row, col+1, col+2,
                              self.company_id.name or '', text_left)
        if self.filter_by:
        ...
        if self.filter_by and self.filter_by == 'by_category':
            ...
            worksheet.write_merge(row, row, col+4, col+5,
                                  self.category_id.name or '', text_left)
    ...
    def create_table_header(self, worksheet, header_style, row, res):
        worksheet.write_merge(row, row+1, 0, 0, 'Product Number', header_style)
        worksheet.write_merge(row, row+1, 1, 3, 'Product', header_style)
        worksheet.write_merge(row, row+1, 4, 5, 'Product Category', header_style)
        worksheet.write_merge(row, row+1, 6, 7, 'Location', header_style)
        worksheet.write_merge(row, row+1, 8, 8, 'Total Qty', header_style)
        worksheet.write_merge(row, row+1, 9, 9, 'Total Value', header_style)
        worksheet.write_merge(row, row, 10, 11, res['6']['name'], header_style)
        worksheet.write(row+1, 10, 'Quantity', header_style)
        worksheet.write(row+1, 11, 'Value', header_style)
        worksheet.write_merge(row, row, 12, 13, res['5']['name'], header_style)
        worksheet.write(row+1, 12, 'Quantity', header_style)
        worksheet.write(row+1, 13, 'Value', header_style)
        worksheet.write_merge(row, row, 14, 15, res['4']['name'], header_style)
        worksheet.write(row+1, 14, 'Quantity', header_style)
        worksheet.write(row+1, 15, 'Value', header_style)
        worksheet.write_merge(row, row, 16, 17, res['3']['name'], header_style)
        worksheet.write(row+1, 16, 'Quantity', header_style)
        worksheet.write(row+1, 17, 'Value', header_style)
        worksheet.write_merge(row, row, 18, 19, res['2']['name'], header_style)
        worksheet.write(row+1, 18, 'Quantity', header_style)
        worksheet.write(row+1, 19, 'Value', header_style)
        worksheet.write_merge(row, row, 20, 21, res['1']['name'], header_style)
        worksheet.write(row+1, 20, 'Quantity', header_style)
        worksheet.write(row+1, 21, 'Value', header_style)
        worksheet.write_merge(row, row, 22, 23, res['0']['name'], header_style)
        worksheet.write(row+1, 22, 'Quantity', header_style)
        worksheet.write(row+1, 23, 'Value', header_style)
        row += 1
        return worksheet, row
    ...
    @api.multi
    def create_table_values(self, worksheet, text_left, text_right, row,
                            res, product_ids, text_right_bold):
        lst = [0, 0, 0, 0, 0, 0, 0]
        ...
        for product in product_ids:
            ...
            worksheet.write_merge(row, row, 4, 5, product.categ_id.display_name, text_left)
            if self.location_ids:
                worksheet.write_merge(row, row, 6, 7, self.location_ids.display_name, text_left)
            else:
                whname = [wh.name for wh in self.warehouse_ids]
                whname = ', '.join(whname)
                worksheet.write_merge(row, row, 6, 7, whname, text_left)
            worksheet.write(row, 8, stock_qty, text_right)
            worksheet.write(row, 9, stock_qty * product.standard_price, text_right)
            col = 10
            for i in range(7)[::-1]:
                from_qty = to_qty = 0
        ...
        worksheet.write_merge(row, row, 0, 7, 'TOTAL', text_right_bold)
        worksheet.write(row, 8, total_qty or 0, text_right_bold)
        worksheet.write(row, 9, total_val or 0, text_right_bold)
        col = 10
        for i in range(7)[::-1]:
    ...
    def get_aging_detail(self):
        ...
        for i in range(7)[::-1]:
            ...
            res[str(i)] = {
                'name': (i != 0 and (str((7 - (i + 1)) * period_length) \
                                        + '-' + str((7 - i) * period_length)) \
    ...
    def print_excel(self):
        ...
        # Print Excel Header
        worksheet, row = self.create_excel_header(worksheet, main_header_style,
                                                  left_header_style, text_left)
        res = self.get_aging_detail()
        worksheet, row = self.create_table_header(worksheet, header_style, row+2, res)
        worksheet, row = self.create_table_values(worksheet, text_left, text_right,
                                                  row, res, product_ids, text_right_bold)
